...
{
    Task<ProfanityCheckResult> CheckAsync(string word, CancellationToken cancellationToken = default);
}

// CircuitOpen is deliberately separate from IsProfane: a tripped circuit breaker means
// ProfanityService could not be asked at all, which is not the same thing as "the word
...
public record ProfanityCheckResult(bool IsProfane, bool CircuitOpen);
